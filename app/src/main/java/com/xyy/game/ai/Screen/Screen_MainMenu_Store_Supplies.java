...
public class Screen_MainMenu_Store_Supplies extends Screen {
    private static final int SUPPLIES_LIMIT = 99;
    ...
    private class Item{
        ...
        private Item(Pixmap ico, String name, String[] description, int price, int counter) {
            mIco = ico;
            mName = name;
    ...
    //列表项的宽度度
    private final int ITEM_WIDTH = 364;
    private final int ITEM_HEIGHT = 206;
    //屏幕中最大显示的行/列数
    private final int ROW_SCREEN = 1;
    private final int COL_SCREEN = 2;
    //列表纵坐标
    private final int LIST_Y = 180;
    //列表总项数
    ...
    public Screen_MainMenu_Store_Supplies(Game game) {
        super(game);
         ...
         */
        mItems = new Item[]{
                new Item(Assets.supply_medkit,"MEDKIT",new String[]{"USE MEDKIT","TO HEAL","YOUR ARMOR"},1,UserDate.sMedkit),
                new Item(Assets.supply_energy,"ENGKIT",new String[]{"USE ENGKIT","TO RESTORE","YOUR ENERGY"},1,UserDate.sEngkit)
        };
                    ...
                    if (event.y > LIST_Y && event.y < LIST_Y_MAX) {
                        ...
                        if (/*!dragging && */event.x >= listX) {
                            ...
                            if (index == this.relativeIndex + startIndex && index < mItemNum) {
                                ...
                                if(UserDate.sCurrency >= item.mPrice && item.mCounter<SUPPLIES_LIMIT){
                                    UserDate.sCurrency -= item.mPrice;
                                    item.mCounter++;
                                }
                            }
                        }
    ...
    public void present(float deltaTime) {
        ...
        g.drawPixmap(Assets.button_tab, 378 + 180 - 8, 68, 180, 0, 180, 68);
    ...
    private void item_list_present(Graphics g, int x, int y, int position, boolean pressed) {
        ...
        g.drawPixmap(Assets.list_item_supplies, x, y);
        ...
        if(item.mCounter == SUPPLIES_LIMIT){
            g.drawPixmap(Assets.button_details_s, x + 195, y + 120, 292, 0, 146, 66);
        }
        else if (pressed)
            g.drawPixmap(Assets.button_details_s, x + 195, y + 120, 146, 0, 146, 66);
        else
            g.drawPixmap(Assets.button_details_s, x + 195, y + 120, 0, 0, 146, 66);
        g.drawText(String.valueOf(item.mPrice), x + 195 + 86, y + 120 + 24 + 16, 0xFFFFFFFF, 24);

        g.drawPixmap(item.mIco, x + 53-8, y + 25);
        g.drawText(item.mName, x + 91, y + 160, 0xFFFFFFFF, 26, Paint.Align.CENTER);
        g.drawText(item.mDescription[0], x + 266, y + 48, 0xFFFFFFFF, 24, Paint.Align.CENTER);
        g.drawText(item.mDescription[1], x + 266, y + 72, 0xFFFFFFFF, 24, Paint.Align.CENTER);
        g.drawText(item.mDescription[2], x + 266, y + 96, 0xFFFFFFFF, 24, Paint.Align.CENTER);
        g.drawText(String.valueOf(item.mCounter), x + 116 , y + 51, 0xFFFFFFFF, 22, Paint.Align.CENTER);
    ...
    public void pause() {
        //TODO: SAVE DATA
        UserDate.sMedkit = mItems[0].mCounter;
        UserDate.sEngkit = mItems[1].mCounter;
    }
